...
unsafe void OnRender(double dt)
{
    ...
    float time = (float)Glfw.GetApi().GetTime();
    float rotation = time * Radians(50.0f);
    Matrix4x4 model = Matrix4x4.CreateRotationX(rotation * 0.5f) * Matrix4x4.CreateRotationY(rotation);
