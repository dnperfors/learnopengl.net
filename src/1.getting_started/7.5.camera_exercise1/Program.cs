...
Camera camera = new(new(0.0f, 0.0f, 3.0f));
...
void OnMouseMove(IMouse mouse, Vector2 mousePosition)
{
    ...
    lastMousePosition = mousePosition;
    camera.ProcessMouseMovement(new(xOffset, yOffset));
}
...
unsafe void OnRender(double dt)
{
    ...
    Matrix4x4 projection = Matrix4x4.CreatePerspectiveFieldOfView(Radians(camera.Zoom), (float)width / (float)height, 0.1f, 100.0f);
